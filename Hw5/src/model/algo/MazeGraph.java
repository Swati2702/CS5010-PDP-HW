...
public class MazeGraph {
  ...
  private Random generator = new Random(32);
  ...
  /**
   * Assigns percent% of locations (at random) gold coins in them that the player can pick.
   * @param percent : % of locations in maze that have @nodeType in them
   * @param nodeType : type of Node - blank, bat or pit
   * @throws IllegalArgumentException if percent is invalid
   * */
  public void assignNodeType(double percent, NodeType nodeType) throws IllegalArgumentException {
    if (percent < 0.0 || percent > 100.0) {
      throw  new IllegalArgumentException("Assignment percent invalid.");
    }
    int count = (int)(adjList.size() * percent) / 100;
    ...
    while (count != 0) {
      ...
      nodeTypeList.put(temp, nodeType);
      count--;
    }
  }
